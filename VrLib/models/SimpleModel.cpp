...
namespace vrlib
{
	...
	template<class VertexFormat>
	void SimpleModel<VertexFormat>::draw(const std::function<void(const glm::mat4&)> &modelviewMatrixCallback, const std::function<bool(const Material&)> &materialCallback)
	{
		if (vao)
		{
			vao->bind();
	...
	template class SimpleModel < gl::VertexP3N2B2T2T2 >;
	template class SimpleModel < gl::VertexP3N2B2T2T2B4B4 >;
